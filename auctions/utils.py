...
def plotutilities(player_dict, price_u, price_v):
    df_u = pd.DataFrame(
        {key: {i: player.u(i) for i in np.arange(0.01, 1, 0.05)} for key, player in player_dict.items()})
    plt.figure()
    df_u.plot()
    plt.xlabel('$b_i$')
    plt.ylabel('$u_i(b_i)$')
    ...
    df_up = pd.DataFrame(
        {key: {i: player.u(i) - price_u * i for i in np.arange(0, 1, 0.05)} for key, player in player_dict.items()})
    plt.figure()
    df_up.plot()
    plt.xlabel('$b_i$')
    ...
    df_v = pd.DataFrame({key: {i: player.u(i) - player.c * (inequality.gen_ent_bc(len(list(player_dict.keys()))))(i) for
                               i in np.arange(0.01, 1, 0.05)} for key, player in player_dict.items()})
    plt.figure()
    df_v.plot()
    plt.xlabel('$b_i$')
    plt.ylabel('$v_i(b_i)$')
    ...
    df_vp = pd.DataFrame({key: {
        i: player.u(i) - player.c * (inequality.gen_ent_bc(len(list(player_dict.keys()))))(i) - price_v * i for i in
        np.arange(0.01, 1, 0.05)} for key, player in player_dict.items()})
    plt.figure()
    df_vp.plot()
    plt.xlabel('$b_i$')
...
def ploteffectc(player_dict, price_include_sw=True, ineq='gini', auct='asc'):
    ...
    cs = np.arange(0., 3, 0.1)
    for c in tqdm(cs):
        for player in player_dict.values():
            player.setc(c)
        if auct == 'asc':
            bv, fp_v = auction.clock_auction(player_dict, descend=False, uorv='v', ineq=ineq)
        else:
            bv, fp_v = auction.clock_auction(player_dict, descend=True, uorv='v', ineq=ineq)
    ...
    df_c_gini = pd.DataFrame({c: [final_prices_c[c], sw[c], iq[c]] for c in cs},
                             index=['Final prices', 'Social Welfare', 'Gini coefficient']).T
    print(df_c_gini)
    plt.figure()
    df_c_gini.plot()
    plt.xlabel('$c$')
...
def bids2str(bids):
    return ''.join(['player:%d, bid: %.3f\n' % (pl, bid) for pl, bid in bids.items()])